...
& $agpGradle --no-daemon --max-workers=2 --console=plain --project-dir $PSScriptRoot `
    ...
    "-Pkotlin.project.persistent.dir=$agpWork/kotlin-$UpstreamVersion" '-Pkotlin.project.persistent.dir.gradle.disableWrite=true' `
    "-PupstreamVersion=$UpstreamVersion" $Task 2>&1 | Tee-Object -FilePath "$agpWork/build-$UpstreamVersion.log"
